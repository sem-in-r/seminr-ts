...
- One runtime dependency: `@compstats/core` (MIT, no transitive deps), supplying R-pinned distributions (`pnorm`/`pchisq`/`pt` and friends) and summaries (`mean`/`sd`/`quantile`) behind `src/math/`'s own names. The package is no longer zero-dependency. Two rules follow: (1) every delegated name needs an **explicit type annotation** — `@compstats/core` 0.5.0's `.d.ts` files use extensionless relative re-exports, which NodeNext resolution rejects, and `skipLibCheck` turns that into a silent `any` (`tests/math/compstats-types.test.ts` enforces it); (2) the optimizer (`src/math/optimize.ts`) is deliberately **not** delegated — see its docstring.
